...
import { ScmRecordFormDialog } from "@/features/scm/ScmRecordFormDialog"
import {
  deleteModuleRecord,
  fetchModuleOverview,
  fetchModuleRecords,
...
import { SCM_LINKED_ROUTES, SCM_PHASE1_FEATURES } from "@/lib/scm-field-groups"
import { keyLabelFromRecord, SCM_MODULE_CODE } from "@/lib/scm-record-schema"
...
export function ScmModulePage() {
  ...
  const overviewQuery = useQuery({
    queryKey: ["erp-modules", SCM_MODULE_CODE, "overview"],
    queryFn: () => fetchModuleOverview(SCM_MODULE_CODE),
  })

  const recordsQuery = useQuery({
    queryKey: ["erp-modules", SCM_MODULE_CODE, "records", activeFeature, search],
    queryFn: () =>
      fetchModuleRecords(SCM_MODULE_CODE, {
  ...
  const canWrite = canAccess(permissions, "scm.records.write")

  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteModuleRecord(SCM_MODULE_CODE, id),
    onSuccess: () => {
  ...
  const records = allRecords.filter((r) => SCM_PHASE1_FEATURES.has(r.feature_code))

  const featureFilters = useMemo(() => {
    return (overview?.features ?? []).filter((f) => SCM_PHASE1_FEATURES.has(f.code))
  }, [overview?.features])

  const defaultFeatureCode = useMemo(() => {
    if (activeFeature !== "all") return activeFeature
    return featureFilters[0]?.code ?? "demand_planning"
  }, [activeFeature, featureFilters])

  return (
    <div className="space-y-4">
      <PageHeader
        title={overview?.module_name ?? "Supply Chain Management (SCM)"}
      ...
      <ContentSheet className="p-4">
        <h2 className="text-sm font-semibold">SCM data domains</h2>
        <p className="mt-1 text-xs text-muted-foreground">
          Each record supports ten categories: master data, inventory, procurement, demand & sales,
          production, logistics, warehouse, financial, compliance, and analytics.
        </p>
